...
function issueUnLoadingCommandToUnit(unitID, area, base)
	SpringGiveOrderToUnit(unitID, CMD.UNLOAD_UNITS, {area.center.x, area.center.y, area.center.z, area.radius}, {})
	SpringGiveOrderToUnit(unitID, CMD.MOVE, {base.x, base.y, base.z}, {"shift"})
end
	...
	for i = 1, #units do
		if SpringGetUnitCommands(units[i], 0) == 0 then
			if isEmpty(units[i]) then
				...
				--perform unloading
				if SpringGetUnitCommands(units[i], 0) == 0 then
					issueUnLoadingCommandToUnit(units[i], parameter.Area, parameter.Base)
				end
			end
		else
			allDone = false
